            ...
            chatData.map((items, index) => (
              ...
              >
                ...
                <h1
                  id={items._id}
                  className={`${
                    items.role === 'model' ? "border border-gray-200 text-black" : "bg-black text-white"
                  }  xl:px-1  xl:py-0.5 xl:w-fit max-w-50  xl:max-w-70   rounded  break-words   `}
                >
                  {items.content}
                </h1>
                 {items.role === 'user' && <img src={UserData.dp? UserData.dp : defaultIcon} className="h-6 w-6  xl:h-6 xl:w-6 object-cover rounded"/>}
              </div>
            ))}
        </div>
      </main>
