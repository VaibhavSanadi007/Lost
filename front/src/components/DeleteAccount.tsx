...
const DeleteAccount = () => {
  ...
  const handleDeleteUser = async ()=>{
    await axios.delete(`${url}/auth/deleteuser`,{withCredentials:true});
  ...
  return (
    <div className="w-1/2 h-fit border border-gray-200 rounded-2xl px-10 py-5 flex flex-col gap-5">
      ...
      <div className="flex gap-4 w-full">
        ...
        <button className="bg-black text-white rounded-xl py-3 active:scale-95 cursor-pointer w-50 flex items-center justify-center gap-2 " onClick={()=>handleDeleteUser()}>
          <AiOutlineUserDelete />
          Yes, delete
          </button>
      </div>
